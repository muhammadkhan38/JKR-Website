...
<form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data" class="rounded-md border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <div class="grid gap-5 lg:grid-cols-2">
        <div>
            <label class="form-label" for="madrasa_name">Madrasa Name</label>
    ...
    <div class="mt-5">
        <label class="form-label" for="short_about">Short About Text</label>
    ...
    <div class="mt-5 grid gap-5 lg:grid-cols-2">
        <div>
            ...
            <p id="logo-file-name" class="mt-2 text-sm text-slate-500">{{ ! empty($settings['logo']) ? 'Current: '.$settings['logo'] : '' }}</p>
        </div>
        <div>
            <label class="form-label" for="homepage_banner">Homepage Banner</label>
            ...
            <p id="banner-file-name" class="mt-2 text-sm text-slate-500">{{ ! empty($settings['homepage_banner']) ? 'Current: '.$settings['homepage_banner'] : '' }}</p>
        </div>
    </div>
    <button class="mt-6 rounded-md bg-emerald-700 px-5 py-3 font-semibold text-white hover:bg-emerald-800">Save Settings</button>
</form>
@endsection
