...
<div class="grid gap-5 lg:grid-cols-2">
    ...
    <div>
        <label class="form-label" for="language">زبان</label>
        <input class="form-input" id="language" name="language" value="{{ old('language', $book->language ?: 'اردو') }}" required>
    </div>
    <div>
        <label class="form-label" for="author_id">مصنف</label>
        <select class="form-input" id="author_id" name="author_id" required>
            <option value="">مصنف منتخب کریں</option>
...
<div class="mt-5">
    <label class="form-label" for="short_description">مختصر وضاحت</label>
...
<div class="mt-5 grid gap-5 lg:grid-cols-2">
    ...
    <div>
        <label class="form-label" for="pdf_file">PDF فائل</label>
        <input class="form-input" id="pdf_file" type="file" name="pdf_file" accept=".pdf" data-file-label="#pdf-file-name" @required(! $book->exists)>
        <p id="pdf-file-name" class="mt-2 text-sm text-slate-500">{{ $book->pdf_file ? 'موجودہ فائل: '.$book->pdf_file : '' }}</p>
    </div>
</div>
<div class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
    @foreach([
        'is_latest' => ['نئی کتاب', false],
        'is_featured' => ['منتخب کتاب', false],
        'is_active' => ['فعال', true],
        'download_allowed' => ['ڈاؤن لوڈ کی اجازت', true],
...
<div class="mt-6 flex gap-3">
    <button class="rounded-md bg-emerald-700 px-5 py-3 font-semibold text-white hover:bg-emerald-800">{{ $book->exists ? 'کتاب اپ ڈیٹ کریں' : 'کتاب شامل کریں' }}</button>
    <a href="{{ route('admin.books.index') }}" class="rounded-md border border-slate-200 px-5 py-3 font-semibold text-slate-700 hover:bg-slate-50">منسوخ کریں</a>
</div>
