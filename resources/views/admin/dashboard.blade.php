...
<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
    @foreach([
        ...
        <div class="admin-panel p-5">
            ...
            <p class="mt-3 text-4xl font-extrabold text-emerald-800">{{ $value }}</p>
        </div>
    @endforeach
</div>

<div class="admin-panel mt-8 overflow-hidden">
    <div class="border-b border-slate-200 px-5 py-4">
        <h2 class="text-lg font-extrabold text-slate-950">{{ __('messages.admin.dashboard_cards.recent_books') }}</h2>
    </div>
        ...
        <table class="admin-table">
            ...
            <tbody>
                @forelse($latestBooks as $book)
                    <tr>
                        <td class="font-bold text-slate-950">{{ $book->localized_title }}</td>
                        <td>{{ $book->author?->localized_name }}</td>
                        <td>{{ $book->category?->localized_name }}</td>
                        <td>
                            <span class="status-badge {{ $book->is_active ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700' }}">{{ $book->is_active ? __('messages.common.active') : __('messages.common.inactive') }}</span>
                        </td>
