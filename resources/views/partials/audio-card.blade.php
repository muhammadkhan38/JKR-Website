<article class="rounded-md border border-emerald-100 bg-white p-4 shadow-sm">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
            ...
            <p class="text-sm text-stone-600">
                {{ $audio->speaker ?: 'Madrasa Lecture' }}
                @if($audio->duration) · {{ $audio->duration }} @endif
            </p>
        </div>
        @if($audio->book)
            <a href="{{ route('books.show', $audio->book) }}" class="text-sm font-semibold text-emerald-700">Related book</a>
        @endif
    </div>
    ...
    @else
        <p class="mt-4 rounded-md bg-stone-100 px-3 py-2 text-sm text-stone-700">Audio file is not uploaded yet.</p>
    @endif
</article>
