...
<article class="flex h-full flex-col overflow-hidden rounded-md border border-emerald-100 bg-white shadow-sm">
    ...
    <div class="flex flex-1 flex-col p-4">
        <div class="mb-3 flex flex-wrap gap-2">
            ...
            @if($book->is_latest)<span class="rounded bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-800">Latest</span>@endif
        ...
        <div class="mt-auto flex flex-wrap gap-2 pt-5">
            <a href="{{ route('books.reader', $book) }}" class="rounded-md bg-emerald-700 px-3 py-2 text-sm font-semibold text-white hover:bg-emerald-800">Read</a>
            <a href="{{ route('books.show', $book) }}" class="rounded-md border border-emerald-200 px-3 py-2 text-sm font-semibold text-emerald-800 hover:bg-emerald-50">Details</a>
            @auth
                <button type="button"
            ...
            @else
                <a href="{{ route('login') }}" class="rounded-md border border-amber-200 px-3 py-2 text-sm font-semibold text-amber-800 hover:bg-amber-50">Login to Bookmark</a>
            @endauth
        </div>
    </div>
</article>
