...
<section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
    <div class="grid gap-8 lg:grid-cols-[320px_1fr]">
        ...
        <div>
            <div class="flex flex-wrap gap-2">
                ...
                @if($book->is_featured)<span class="rounded bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800">Featured</span>@endif
            </div>
            <h1 class="mt-4 text-3xl font-bold text-emerald-950 sm:text-4xl">{{ $book->title }}</h1>
            <p class="mt-3 text-stone-700">By <strong>{{ $book->author?->name }}</strong> in <a class="font-semibold text-emerald-700" href="{{ route('categories.show', $book->category) }}">{{ $book->category?->name }}</a></p>
            <p class="mt-5 text-lg leading-8 text-stone-700">{{ $book->short_description }}</p>
            <div class="mt-6 flex flex-wrap gap-3">
                <a href="{{ route('books.reader', $book) }}" class="rounded-md bg-emerald-700 px-5 py-3 font-semibold text-white hover:bg-emerald-800">Read Online</a>
                @if($book->download_allowed)
                    <a href="{{ route('books.download', $book) }}" class="rounded-md border border-emerald-200 px-5 py-3 font-semibold text-emerald-800 hover:bg-emerald-50">Download PDF</a>
                @endif
                @auth
                    ...
                    <button type="button"
                        ...
                        {{ $isBookmarked ? 'Remove Bookmark' : 'Bookmark Book' }}
                    </button>
                @else
                    <a href="{{ route('login') }}" class="rounded-md border border-amber-200 px-5 py-3 font-semibold text-amber-800 hover:bg-amber-50">Login to Bookmark</a>
                @endauth
            </div>
            @if($book->description)
    ...
    @if($book->audios->isNotEmpty())
        <div class="mt-12">
            ...
            <h2 class="section-heading">Related Audio</h2>
    ...
    @if($relatedBooks->isNotEmpty())
        <div class="mt-12">
            <p class="section-kicker">More From Category</p>
